...
1. eclipse直接创建Dynamic web Project工程，在启动工程时，选用我们自己的含有漏洞的tomcat版本，将tomcat/conf/web.xml添加readonly参数：
...
```
2.启动工程，在浏览器访问，通过burpsuite进行抓包：

![Tomcat]({{ '/styles/images/tomcat/12.png' | prepend: site.baseurl  }})

3.转发数据包，右键Send to Repeater，构造payload，实现任意命令执行：
...
4.点击go按钮，创建一句话木马文件，创建成功后，访问相应文件，执行任意命令：
